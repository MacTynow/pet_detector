...
class Classifier():
    ...
    def set_label_colours(self):
        ...
        self.label_colours = np.random.randint(
    ...
    def load_model_to_memory(self):
        ...
        self.network = cv2.dnn.readNetFromDarknet(
            self.model_config_path, self.model_weights_path)
        self.layer_names = [self.network.getLayerNames()[i[0] - 1]
    ...
    def initialize_video_writer(self, sample_frame, fps=30):
        ...
        self.video_writer = cv2.VideoWriter(
            self.output_video_path, writer_class, fps, height_width, True)

    def initialize_live_video_stream(self):
        self.video_stream = VideoStream(src=0, usePiCamera=True).start()
        time.sleep(2.0)
        self.fps = FPS().start()
    ...
    def build_bounding_box(self, detection):
        ...
        box = detection[0:4] * np.array([self.frame_width,
                                        self.frame_height, self.frame_width, self.frame_height])
        (center_x, center_y, prediction_width,
         prediction_height) = box.astype("int")
        # get top left corner
    ...
    def draw_bounding_box(self, frame, i, x, y, w, h):
        ...
        text = "{}: {:.4f}".format(
            self.labels[self.class_id_list[i]], self.confidence_list[i])
        font_scale = 1.5
        cv2.putText(frame, text, (x, y - 5),
                    cv2.FONT_HERSHEY_SIMPLEX, font_scale, colour, 3)
